...
//	List<Object> findAllProposedPolicies(int customer_id);
	...
	@Query("select p from Policy p join p.vehicle v join v.customer c where c.id=?1 and p.status=?2")
	List<Object> findAllProposedPolicies(int customer_id,PolicyStatus status);
